...
/// </summary>
namespace Console.Core.CommandSystem.Builder.BuiltIn.EnumAutoFill
{
    /// <summary>
    ...
    public class EnumAutoFillProvider : AutoFillProvider
    {
        ...
        public override string[] GetAutoFills(AbstractCommand cmd, int paramNum, string start)
        {
            ...
            if (cmd.MetaData.Count >= paramNum)
            {
                if (!(cmd.MetaData[paramNum - 1].Attributes.FirstOrDefault(x => x is EnumAutoFillAttribute) is
                          EnumAutoFillAttribute ret))
                {
                    return new string[0];
                }
