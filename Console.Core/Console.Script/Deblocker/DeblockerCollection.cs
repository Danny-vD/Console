...
namespace Console.Script.Deblocker
{
    ...
    public static class DeblockerCollection
    {
        ...
        {
            string clean = RemoveComments(
                                          content,
                                          DeblockerSettings.CommentPrefix,
                                          ConsoleCoreConfig.NewLine.ToString(),
                                          0
                                         );
            clean = RemoveComments(
                                   clean,
                                   DeblockerSettings.CommentMultiPrefix,
                                   DeblockerSettings.CommentMultiPostfix,
                                   DeblockerSettings.CommentMultiPostfix.Length
                                  );
        ...
        private static int GetEndOfLine(string content, int start)
        {
            for (int i = start; i < content.Length; i++)
            {
                ...
                {
                    int closeIdx = ConsoleCoreConfig.FindClosing(
                                                                 content,
